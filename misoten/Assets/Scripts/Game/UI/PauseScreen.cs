		...
		if (!coinFoObj.GetComponent<CoinFO>().GetIsStartingCoinFO())
		{
			...
			for (int i = 0; i < playerObj.Length; i++)
			{
				...
				if (!pauseObj.GetComponent<Pause>().pausing
					&& GamePad.GetButtonDown(GamePad.Button.Start, playerObj[i].GetComponent<Player>().GetControllerInformation().controllerNumber))
				{
				...
				{
					// セレクトカーソルの更新
					if (GamePad.GetAxis(GamePad.Axis.LeftStick, playerObj[i].GetComponent<Player>().GetControllerInformation().controllerNumber).y == 1.0f && !selectCursorFlag && selectCursor == 1)
					{
						selectCursor = 0;
					...
					if (GamePad.GetAxis(GamePad.Axis.LeftStick, playerObj[i].GetComponent<Player>().GetControllerInformation().controllerNumber).y == -1.0f && !selectCursorFlag && selectCursor == 0)
					{
						selectCursor = 1;
						selectCursorFlag = true;
					...
					// ゲーム画面へ戻る
					if (GamePad.GetButtonDown(GamePad.Button.Start, playerObj[i].GetComponent<Player>().GetControllerInformation().controllerNumber)
						|| GamePad.GetButtonDown(GamePad.Button.A, playerObj[i].GetComponent<Player>().GetControllerInformation().controllerNumber))
					{
					...
					// 決定
					if (GamePad.GetButtonDown(GamePad.Button.B, playerObj[i].GetComponent<Player>().GetControllerInformation().controllerNumber))
					{
						// ポーズ中の項目
						switch (selectCursor)
						{
